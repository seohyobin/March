...
export default function SignUpComponent(props) {
    ...
    const onChangeUserId=(e)=>{ 
        ...
        const {value}= e.target; //e.target.value랑 같은거// 비구조화 구조분할할당
        const regEx1 = /[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]/g;
    ...
    const onClickUserIdOk=(e)=>{
        e.preventDefault();
        const value = state.아이디; //상태관리 값을 가져와서 변수에 대입
        const regEx1 = /[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]/g;
        const regEx2 = /.{6,16}/g;  //입력제한 6자 이상 16자 이하
        const regEx3 = /[가-힣ㄱ-ㅎㅏ-ㅣ]/g;  //한글
        const regEx4 = /[A-Za-z]+[0-9]*/g;  //영문필수->한글자 이상+,  무조건 나와야함  숫자선택0자이상 *  \d 도 숫자
    ...
    const onChangeUserPw1=(e)=>{
        ...
        //+ 1자이상 반드시
        //* 0자이상
        //[] 범위 문자또는 숫자 , 특수문자 등의 범위 range
        //() 그룹
        //특수문자는 \-  \]  \\  3가지는 반드시 이스케이프(Escaped) 문자 처리
        //const regExp1 = /.{10,16}/g; //1)10자이상 16자이하 true이면 정상 헐렁한 수량자 
        const regExp1 = /^.{10,16}$/g; //1)10자이상 16자이하 true이면 정상 헐렁한 수량자 => ^$ 시작과 끝 넣으면 엄격하게 검사!!!!
        const regExp2 = /([A-Za-z]+[0-9]+)+|([0-9]+[A-Za-z]+)+|([A-Za-z]+[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+)+|([`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+[A-Za-z]+)+|([0-9]+[`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+)+|([`~!@#$%^&*()\-_=+[{\]}\\|;:'",<.>/?]+[0-9]+)+/g; //true 이면 정상 
        const regExp3 = /[가-힣ㄱ-ㅎㅏ-ㅣ]/g; // 한글 false 이면 정상
        const regExp4 = /\s/g; //공백 false 이면 정상
        const regExp5 = /(\d)\1\1/g; //동일한 숫자 연속 3자 이상===false 이면 정상
        // const regExp6 = /(.)\1\1/g; //동일한 문자,숫자 연속 3자 이상
    ...
    //6.이메일 입력상자 onChange이벤트
        //입력제한조건 이메일 형식으로 입력해 주세요.
        //1)공백불가
        //2)@ . 무조건 나와
        //3). @ 1자 사용
       ...
       if(value===''){
        ...
        .then((res)=>{//성공
            ...
            try{
                ...
                console.log(`axios오류 메시지 ! ${err}`);
            }
            res.data.map();

        })
        .catch((err)=>{//실패
            console.log(`axios실패 메시지 ! ${err}`);
        });
       }
    }
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <div>
                                            ...
                                            <p className={`error-msg ${state.isEmailError? ' on' : ''}`}> 이메일{state.isEmailMsg}</p>
                                            <button 
...
SignUpComponent.defaultProps = {
    회원가입: {// id, pw, name, emain, hp ,address ,gender ,birth ,service ,chooga ,sign_up_date
