...
export default function SignUpComponent({회원가입, timer,timerCounterfn,mapAddressFn}) {
   ...
   // const regExp = /[^\d]/g  
    ...
    // })
        ...
        const regExp = /^([0-9]{3})([0-9]{3,4})([0-9]{3})$/g
        ...
        newFormData.append('user_hp', state.휴대폰.replace(regExp,'$1-$2-$3'));
        ...
        newFormData.append('user_gaib_date',`${new Date().getFullYear()}-${new Date().getMonth+1}-${new Date().getDay()}`);



        axios({
            url:'http://luck1409.dothome.co.kr/signup_db/form_again.php',
            method:'POST',
            data:newFormData
        })
        .then((res)=>{
            if(res.status===200){
                ...
                setState({
                    ...state,
                    isConfirmModal:true,
                    confirmModalMsg:'마켓컬리 회원가입을 축하드립니다'
                })
             }
