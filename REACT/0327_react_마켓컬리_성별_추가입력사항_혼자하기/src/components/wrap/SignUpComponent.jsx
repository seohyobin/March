...
export default function SignUpComponent({회원가입, timer,timerCounterfn,mapAddressFn}) {
    ...
    const onClickChoogaIdokBtn =(e)=>{
        ...
        let isConfirmModal =false;
        let confirmModalMsg = '';
        let isChoogaIdOkDisabled = false;
        ...
        .then((res)=>{
            ...
            if(result.includes(true)){
                ...
                isChoogaIdOkDisabled = true;
            }
            else{
                isConfirmModal =true;
                confirmModalMsg='존재하지 않는 아이디입니다.';
                isChoogaIdOkDisabled = false;
            }
            setState({
                ...state,
                isConfirmModal:isConfirmModal,
                confirmModalMsg:confirmModalMsg,
                isChoogaIdOkDisabled:isChoogaIdOkDisabled
            })
        })
        .catch((err)=>{
            console.log(`AXIOS 실패!!!!  ${err}`);
                                        ...
                                        <div>
                                            ...
                                            <div className="center-box">
                                              <button className={`addr-search-btn ${state.isAddress?' on' :''}`} onClick={onClickAddrPopupOpenApi} type='button'>주소검색</button>
                                                <input onFocus={onChangeAddr1} onChange={onChangeAddr1} value={state.주소1} type='text'className={`${state.isAddress?' on':''}`} name='user_address1' id='userAddress1' placeholder='검색   주소1' />
                                                <input onFocus={onChangeAddr2} onChange={onChangeAddr1} value={state.주소2} type='text'className={`${state.isAddress?' on':''}`} name='user_address2' id='userAddress2' placeholder='나머지 주소2' />
                                                <h5>샛별배송</h5> 
                                    ...
                                    <li>
                                        <div>
...

                                            {
                                                state.isChooga1 && (
                                                    <button 
                                                    disabled={state.isChoogaIdOkDisabled}
                                                    onClick={onClickChoogaIdokBtn}
                                                    className={`chooga-idok-btn ${state.isChoogaIdOkDisabled ? ` on` : ''}`}
                                                    >아이디 확인</button>
                                                )
                                            }
                                        </div>
